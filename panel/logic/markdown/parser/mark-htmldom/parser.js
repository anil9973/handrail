...
export class MarkdownParser {
	...
	/** @param {AsyncGenerator<any, void, unknown>} stream*/
	async *parseStream(stream) {
		for await (const chunk of stream) {
			...
			this.tokenizer.consume(chunkText);
	...
	insertStartMarker(marker) {
		...
		if (this.blockElemStack.at(-1).tagName === "BLOCKQUOTE") return this.currentLine.push(this.blockElemStack.at(-1));
	...
	insertFenceBlock(content, isEnd, blockType) {
		this.blockElemStack.length > 1 && this.blockElemStack.pop();
		if (this.openBlock) {
			...
			blockType && (this.openBlock.className = blockType);
			this.openBlock.appendChild(new Text(content));
