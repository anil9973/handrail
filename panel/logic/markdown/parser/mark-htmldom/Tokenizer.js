import { CharCode, Markers, Sequences, State, TableDividers, TwinMarkers } from "./enums.js";
...
export class Tokenizer extends EventEmitter {
	...
	verifyTableSyntax(char) {
		...
		if (char === CharCode.Pipe) {
			...
			while (this.buffer[++crtIndex] !== CharCode.LineBreak) {
				char = this.buffer[crtIndex];
				if (TableDividers.has(char) && char === CharCode.Pipe) headCount--;
			}
	...
	stateInFenceBlock(blockType) {
		...
		this.emit("fenceblock", isEnd ? preContent.slice(0, -3) : preContent, isEnd, blockType);
	...
	stateBeforeFenceBlock() {
		...
		if (this.buffer[++this.index] === CharCode.BackTick && this.buffer[++this.index] === CharCode.BackTick) {
			this.sectionStart = this.index + 1; // skip last `
			this.fastForwardTo(CharCode.LineBreak);
			const blockType = this.buffer.slice(this.sectionStart, this.index);
			this.sectionStart = this.index;
			this.state = State.InFenceBlock;
			this.stateInFenceBlock(blockType);
		}
	}
	...
	blockMarkers = {
		...
		// biome-ignore format:
		[CharCode.Asterisk]: () =>
			this.buffer[this.index + 1] === CharCode.Asterisk
				? this.stateInlineMarker()
		...
		[CharCode.OpeningSqrBracket]: () =>
			this.buffer[++this.index] === CharCode.Circumflex
				? this.stateInFootnote()
				: this.buffer[this.index] === CharCode.OpeningSqrBracket
				? this.stateInWikiLink()
				: this.stateBeforeLinkUrl(),
	};
	...
	markers = {
		...
		[CharCode.OpeningSqrBracket]: () => {
			this.sendTextData();
			// biome-ignore format:
			this.buffer[this.index + 1] === CharCode.Circumflex
				? this.stateInFootNoteRef()
				: this.buffer[this.index + 1] === CharCode.OpeningSqrBracket
				? this.stateInWikiLink()
				: this.stateBeforeLinkUrl();
		},
		[CharCode.Percent]: () => {
	...
	stateBeforeLinkUrl() {
		...
		// biome-ignore format:
		if (
			this.buffer[this.index] === CharCode.ExclamationMark &&
			this.buffer[++this.index] === CharCode.OpeningSqrBracket
		) {
			this.sectionStart = this.index;
			this.fastForwardTo(CharCode.ClosingSqrBracket);
